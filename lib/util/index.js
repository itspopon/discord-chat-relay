...
const getTimestamp = () => {
  ...
  // Time
  str += d.getUTCHours() < 10 ? `0${d.getUTCHours().toString()}` : d.getUTCHours().toString();
  str += ':';
  str += d.getUTCMinutes() < 10 ? `0${d.getUTCMinutes().toString()}` : d.getUTCMinutes().toString();
  str += ':';
  str += d.getUTCSeconds() < 10 ? `0${d.getUTCSeconds().toString()}` : d.getUTCSeconds().toString();
  // Timezone
  str += ' UTC';
...
const getDateAndTimestamp = () => {
  ...
  // Day
  str += d.getDate() < 10 ? `_0${d.getDate().toString()}_` : `_${d.getDate().toString()}_`;
  // Time
  str += d.getUTCHours() < 10 ? `0${d.getUTCHours().toString()}` : d.getUTCHours().toString();
  str += ':';
  str += d.getUTCMinutes() < 10 ? `0${d.getUTCMinutes().toString()}` : d.getUTCMinutes().toString();
...
const unHtml = (() => {
  ...
  return function unHtml(s) {
    return s.replace(/<.*?>/g, '').replace(/&(quot|amp|lt|gt);/g, (_, $1) => replacements[$1]);
  };
})();

...
const saveLog = (filename, object) => {
  fs.writeFile(
    ...
    util.inspect(object),
    err =>
      (err ? console.log(err) : console.log(`The ${filename} logfile was saved!`))
  );
};

const appendLog = (filename, object) => {
  fs.appendFile(
    `${__dirname}/logs/${filename}.json`,
    util.inspect(object),
    err =>
...
class TeraHandler {
  ...
  teraToDiscordIPC() {
    ...
    this.sysmsg.on('SMT_GACHA_REWARD', (params) => {
      saveLog('SMT_GACHA_REWARD', params);
      if (this.allGuildies.indexOf(params.UserName) !== -1) {
        this.ipc_tera.send(
          'sysmsg',
          `* ${params.UserName} obtained < ${this.conv(params.randomItemName)} > x ${params.randomItemCount} from <${this.conv(params.gachaItemName)}>.`,
          {
            codeStyle: 'md'
          }
        );
    ...
    this.dispatch.hook('S_GUILD_QUEST_LIST', 1, (event) => {
      ...
      if (!_.isEqual(this.guildQuestList, event.quests)) {
        ...
        for (let i = 0, len = this.guildQuestList.length; i < len; i++) {
          // Check for rally
          if (
            this.guildQuestList[i].isRally === 2
            || this.guildQuestList[4] === 4
          ) {

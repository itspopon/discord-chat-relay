...
class TeraHandler {
  constructor(dispatch, config) {
    ...
    this.ipc_tera = new IpcClient(this.socket, (event, ...args) => {
      switch (event) {
        case 'fetch': {
          for (let typename in this.GINFO_TYPE) {
            this.requestGuildInfo(this.GINFO_TYPE[typename]);
          }
          break;
        }
        ...
        case 'chat': {
          ...
          this.sendOrQueue('C_CHAT', 1, {
            channel: 2,
            message:
              '<FONT>' + this.escape(`<${author}> ${message}`) + '</FONT>'
          });
          break;
        }

        // I think this allows us to whisper someone from discord o.O?
        case 'whisper': {
          const [target, message] = args;
          this.sendOrQueue('C_WHISPER', 1, {
            target: target,
  ...
  init() {
    ...
    setInterval(() => {
      ...
      for (let typename in this.GINFO_TYPE) {
        const type = this.GINFO_TYPE[typename];
        if (
  ...
  teraToDiscordIPC() {
    ...
    this.sysmsg.on('SMT_GUILD_LOG_LEAVE', params => {
      this.ipc_tera.send(
        'sysmsg',
        `\`\`\`cs\n# ${params['UserName']} has left the guild.\n\`\`\``
      );
      this.requestGuildInfo(this.GINFO_TYPE.members);
    });
    ...
    this.sysmsg.on('SMT_GUILD_MEMBER_LOGON', params => {
      this.ipc_tera.send(
        'sysmsg',
        `<:nya:308670487117037569> \`${
          params['UserName']
        } logged in. Message: ${params['Comment']}\``
      );
      this.requestGuildInfo(this.GINFO_TYPE.members);
    });
    ...
    this.sysmsg.on('SMT_ACCOMPLISH_ACHIEVEMENT_GRADE_GUILD', params => {
      this.ipc_tera.send(
        'sysmsg',
        `\`\`\`md\n< ${params['name']} earned a ${this.conv(
          params['grade']
        )}. >\n\`\`\``
      );
    });
    ...
    this.sysmsg.on('SMT_GQUEST_NORMAL_ACCEPT', params => {
      ...
      this.ipc_tera.send(
        ...
        `Took ${this.conv(params['guildQuestName'])}`,
        `.re ${this.conv(params['guildQuestName'])}`
      );
    });

    this.sysmsg.on('SMT_GQUEST_NORMAL_COMPLETE', params => {
      this.ipc_tera.send(
        'sysmsg_gquestcomplete',
    ...
    this.sysmsg.on('SMT_GQUEST_NORMAL_CANCEL', params => {
      this.ipc_tera.send(
        'sysmsg',
        `\`\`\`ini\n${params['userName']} canceled [${this.conv(
          params['guildQuestName']
        )}].\n\`\`\``
      );
      this.requestGuildInfo(this.GINFO_TYPE.quests);
    });
    ...
    this.sysmsg.on('SMT_GQUEST_NORMAL_END_NOTICE', params => {
      this.ipc_tera.send(
        'sysmsg',
        `@Staff The current guild quest ends in 10min.`
      ); // ?
    });
    ...
    this.sysmsg.on('SMT_GQUEST_NORMAL_CARRYOUT', params => {
      ...
      this.ipc_tera.send(
        'sysmsg',
        `\`\`\`ini\n${params['userName']} advanced [${this.conv(
          params['guildQuestName']
          ...
          '```md\n* ' +
            this.escapeHtml(
              `${params['UserName']} has successfully enchanted ` +
                `(+${params['Added']}) < ${this.conv(params['ItemName'])} >.`
            ) +
            '\n```'
        );
      }
          ...
          '```md\n* ' +
            this.escapeHtml(
              `* ${params['UserName']} obtained < ${this.conv(
                ...
                `${params['randomItemCount']} from <${this.conv(
    ...
    this.dispatch.hook('S_GUILD_QUEST_LIST', 1, event => {
      ...
      if (!_.isEqual(this.quests, currQuests)) {
        ...
        this.ipc_tera.send('quest', this.quests);
      }
    });
  ...
  escape(str) {
    const words = ['.com', 'fag', 'mmoc', 'molest', 'nigg'];

    const wordRegex = new RegExp(
      '(' + words.map(this.escapeRegExp).join('|') + ')',
      'gi'
    );

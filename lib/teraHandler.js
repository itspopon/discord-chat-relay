    ...
    this.dispatch.hook('S_UPDATE_GUILD_QUEST_STATUS', 1, (event) => {
      fs.writeFile(
        ...
        err =>
          (err
            ? console.log(err)
            : console.log('The sUpdateGuildQuestStatus logfile was saved!'))
      );
    ...
    this.dispatch.hook('S_GUILD_QUEST_LIST', 1, (event) => {
      fs.writeFile(
        ...
        err =>
          (err
            ? console.log(err)
            : console.log('The sGuildQuestList logfile was saved!'))
      );

      if (_.isEmpty(this.guildQuestList)) {
        this.guildQuestList = event.quests;
      } else if (!_.isEqual(this.guildQuestList, event.quests)) {
        const questDiff = _.reduce(
          this.guildQuestList,
          (result, value, key) =>
            (_.isEqual(value, event.quests[key]) ? result : result.concat(key)),
          []
        );

        fs.writeFile(
          `${__dirname}/logs/sGuildQuestList_questDiff_${Date.now()}.json`,
          util.inspect(questDiff),
          err =>
            (err
              ? console.log(err)
              : console.log('The sGuildQuestListDiff logfile was saved!'))
        );
          ...
          if (this.guildQuestList[i].isRally === 2) {
            ...
            fs.writeFile(
              ...
              err =>
                (err
                  ? console.log(err)
                  : console.log('The rallyFound logfile was saved!'))
            );
    ...
    this.dispatch.hook('S_GUILD_INFO', 1, (event) => {
      fs.writeFile(
        ...
        err =>
          (err
            ? console.log(err)
            : console.log('The sGuildInfo logfile was saved!'))
      );
    ...
    this.dispatch.hook('S_GUILD_MEMBER_LIST', 1, (event) => {
      fs.writeFile(
        ...
        err =>
          (err
            ? console.log(err)
            : console.log('The sGuildMemberList logfile was saved!'))
      );

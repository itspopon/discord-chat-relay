...
class TeraHandler {
      ...
      switch (event) {
        ...
        case 'chat': {
          ...
          this.sendOrQueue('C_CHAT', 1, {
            channel: 2,
            message: `<FONT>${this.escape(`<${author}> ${message}`)}</FONT>`
          });
          break;
        }
        ...
        case 'info': {
          ...
          this.sendOrQueue('C_CHAT', 1, {
            channel: 2,
            message: `<FONT>* ${this.escape(message)}</FONT>`
          });
          break;
        }
  ...
  teraToDiscordIPC() {
    ...
    this.dispatch.hook('S_UPDATE_GUILD_QUEST_STATUS', 1, (event) => {
      fs.writeFile(
        `${__dirname}/logs/sUpdateGuildQuestStatus.json`,
        util.inspect(event),
        err =>
          (err
            ? console.log(err)
            : console.log('The sUpdateGuildQuestStatus was saved!'))
      );
      // @@@
      console.log(
        `[${U.getTimestamp()}][teraHandler.js] Gquest status update. Event: `,
        event
      );
    ...
    this.sysmsg.on('SMT_QUEST_COUNT_MAX', (params) => {
      // @@@
      console.log(
        `[${U.getTimestamp()}][teraHandler.js] SMT_QUEST_COUNT_MAX params: `,
        params
      );
    });
    ...
    this.sysmsg.on('SMT_GQUEST_NORMAL_ACCEPT', (params) => {
      // @@@
      console.log(
        `[${U.getTimestamp()}][teraHandler.js] SMT_GQUEST_NORMAL_ACCEPT params: `,
        params
      );
      this.ipc_tera.send(
        'sysmsg',
    ...
    this.sysmsg.on('SMT_GQUEST_NORMAL_COMPLETE', (params) => {
      // @@@
      console.log(
        `[${U.getTimestamp()}][teraHandler.js] SMT_GQUEST_NORMAL_COMPLETE params: `,
        params
      );
      this.ipc_tera.send(
        'sysmsg',
    ...
    this.sysmsg.on('SMT_MAX_ENCHANT_SUCCEED', (params) => {
      if (this.allGuildies.indexOf(params.UserName) !== -1) {
        this.ipc_tera.send(
          'sysmsg',
          `* ${this.escapeHtml(`${params.UserName} has successfully enchanted (+${
              params.Added
    ...
    this.sysmsg.on('SMT_GACHA_REWARD', (params) => {
      if (this.allGuildies.indexOf(params.UserName) !== -1) {
        this.ipc_tera.send(
          'sysmsg',
          `* ${this.escapeHtml(`* ${params.UserName} obtained < ${this.conv(params.randomItemName)} > x ${params.randomItemCount} from <${this.conv(params.gachaItemName)}>.`)}`,
          'md'
        );
      }
    });
    ...
    this.dispatch.hook('S_GUILD_QUEST_LIST', 1, (event) => {
      fs.writeFile(
        `${__dirname}/logs/sGuildQuestList.json`,
        util.inspect(event),
        err =>
          (err
            ? console.log(err)
            : console.log('The sGuildQuestList file was saved!'))
      );

      fs.writeFile(
        `${__dirname}/logs/sGuildQuestList_questDiff_${Date.now()}.json`,
        util.inspect(difference(event.quests, this.guildQuestList)),
        err =>
          (err
            ? console.log(err)
            : console.log('The sGuildQuestListDiff file was saved!'))
      );
      ...
      if (!_.isEqual(this.guildQuestList, event.quests)) {
        ...
        for (let i = 0, len = this.guildQuestList.length; i < len; i++) {
          // Check for rally
          if (this.guildQuestList[i].isRally === 2) {
            console.log('@@@ Rally found!');

            fs.writeFile(
              `${__dirname}/logs/rallyFound_${Date.now()}.json`,
              util.inspect(this.guildQuestList[i]),
              err =>
                (err
                  ? console.log(err)
                  : console.log('The rallyFound file was saved!'))
            );
          } else if (this.guildQuestList[i].isRally === 1) {
            console.log('@@@ isRally value is 1');
          }

          // Check for guild quest expiry
          if (
            this.guildQuestList[i].status > 0
            && this.guildQuestList[i].timeRemaining < 28800
          ) {
            console.log(
              '@@@ THIS GUILD QUEST IS GOING TO EXPIRE in 4 hours: ',
              this.conv(this.guildQuestList[i].name)
            );
          }
        }
      }
      ...
      // @@@
      console.log(
        `[${U.getTimestamp()}][teraHandler.js] S_GUILD_QUEST_LIST Completed Quests: (`,
    ...
    this.dispatch.hook('S_GUILD_INFO', 1, (event) => {
      fs.writeFile(
        `${__dirname}/logs/sGuildInfo.json`,
        util.inspect(event),
        err =>
          (err ? console.log(err) : console.log('The sGuildInfo file was saved!'))
      );
    ...
    this.dispatch.hook('S_GUILD_MEMBER_LIST', 1, (event) => {
      fs.writeFile(
        `${__dirname}/logs/sGuildMemberList.json`,
        util.inspect(event),
        err =>
          (err
            ? console.log(err)
            : console.log('The sGuildMemberList was saved!'))
      );

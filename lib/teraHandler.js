...
class TeraHandler {
  ...
  saveGuild(guildData) {
    try {
      this.models.TeraGuild.findByIdAndUpdate(this.oids.guild, { guildData }, { upsert: true }, () => {
  ...
  teraToDiscordIPC() {
    this.dispatch.hook('S_LOGIN', 10, event => {
      ...
      this.saveLog(`[${U.getTimestamp()}][client] Logged into character: ${this.myName}`);
    });
    ...
    this.sysmsg.on('SMT_GC_MSGBOX_APPLYLIST_2', params => {
      ...
      this.ipc_tera.send('firewall', `> ${params.Name}'s guild application was rejected.`, {
        codeStyle: 'diff'
      });

      this.ipc_tera.send('gamechat', `> ${params.Name}'s guild application was rejected.`, {
        codeStyle: 'diff'
      });
    ...
    this.sysmsg.on('SMT_GC_MSGBOX_APPLYRESULT_1', params => {
      ...
      this.ipc_tera.send('gamechat', `+ ${params.Name1} accepted ${params.Name2} into the guild.`, {
        codeStyle: 'diff'
      });
    ...
    this.sysmsg.on('SMT_GC_MSGBOX_APPLYRESULT_2', params => {
      ...
      this.ipc_tera.send('gamechat', `> ${params.Name1} rejected ${params.Name2}'s guild application.`, {
        codeStyle: 'diff'
      });
    ...
    this.sysmsg.on('SMT_GUILD_LOG_LEAVE', params => {
      ...
      this.ipc_tera.send('gamechat', `- ${params.UserName} has left the guild.`, {
        codeStyle: 'diff'
      });
    ...
    this.sysmsg.on('SMT_GUILD_LOG_BAN', params => {
      ...
      this.ipc_tera.send('gamechat', `- ${params.UserName} was kicked out of the guild.`, {
        codeStyle: 'diff'
      });
    ...
    this.sysmsg.on('SMT_GUILD_MEMBER_LOGON', params => {
      ...
      this.ipc_tera.send(
        'gamechat',
        `<:Aw:489893064005910554> \`${params.UserName} logged in. Message: ${_.unescape(params.Comment)}\``
      );
      this.logOn(params.UserName);
    });

    this.sysmsg.on('SMT_GUILD_MEMBER_LOGON_NO_MESSAGE', params => {
      this.ipc_tera.send('gamechat', `<:Aw:489893064005910554> \`${params.UserName} logged in.\``);
      this.logOn(params.UserName);
    });

    this.sysmsg.on('SMT_GUILD_MEMBER_LOGOUT', params => {
      this.ipc_tera.send('gamechat', `<:Cry:489897676704317450> \`${params.UserName} logged out.\``);
      this.logOff(params.UserName);
    });
    ...
    this.sysmsg.on('SMT_GQUEST_URGENT_NOTIFY', params => {
      ...
      if (today !== 2 && today !== 5) {
        this.ipc_tera.send('guildchat', `<@&425025256999354368> BAM spawning soon! <@&345648353884897293> Take the gquest!`);
      } else {
        this.ipc_tera.send(
          'guildchat',
          `<@&425025256999354368> (PVP) BAM spawning soon! <@&345648353884897293> Take the gquest!`
        );
      }
    });

...
class TeraHandler {
  ...
  teraToDiscordIPC() {
    ...
    this.dispatch.hook('S_CHAT', 2, (event) => {
      // @@@
      console.log('Channel: {', event.channel, '} | Author: {', event.authorName, '} | Message: ', event.message);
    ...
    this.sysmsg.on('SMT_MAX_ENCHANT_SUCCEED', (params) => {
      if (this.allGuildies.indexOf(params.UserName) !== -1) {
        this.ipc_tera.send(
          'sysmsg',
          `* ${this.escapeHtml(`${params.UserName} has successfully enchanted (+${params.Added}) < ${this.conv(params.ItemName)} >.`)}`,
          'md',
        );
      }
    });
    ...
    this.sysmsg.on('SMT_GACHA_REWARD', (params) => {
      if (this.allGuildies.indexOf(params.UserName) !== -1) {
        this.ipc_tera.send(
          'sysmsg',
          `* ${this.escapeHtml(`* ${params.UserName} obtained < ${this.conv(params.randomItemName)} > x ${
    ...
    this.dispatch.hook('S_GUILD_QUEST_LIST', 1, (event) => {
      // @@@
      console.log('[teraHandler.js] Completed Quests: (', event.completedQuests, '/', event.maxQuests, ')');
      ...
      const currQuests = activeQuests.map((quest) => {
        ...
        switch (quest.size) {
          ...
          case 2:
            ...
            break;
          default:
            console.warn('[teraHandler.js] Unknown size: ', quest.size);
            break;
        }
        const name = this.conv(quest.name);
        const nameWithSize = `${name} ${questSize}`;
        // @@@
        console.log('[teraHandler.js] Quest name: ', name, ' | size: ', quest.size);

        if (quest.targets.length === 1 && name !== 'Crafting Supplies') {
          const [target] = quest.targets;
          return { name: nameWithSize, completed: target.completed, total: target.total };
        }
        ...
        return { name: nameWithSize, targets };
      });

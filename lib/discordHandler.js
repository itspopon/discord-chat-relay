...
class DiscordHandler {
  constructor(dispatch, config) {
    ...
    this.discordBot.on('ready', () => {
      ...
      this.whisperChannel = U.getTextChannel(this.server, this.config.channels.whisper);
    ...
    this.discordBot.login(this.config.token).catch(reason => {
      console.error(`[${U.getTimestamp()}][Relay] failed to login with bot:`, reason);
      this.discordBot.destroy();
      process.exit();
    });
  }
  ...
  discordIPCToDiscordMsg() {
    ...
    this.ipc_discord.on('botState', state => {
      if (state.loggedIn) {
        ...
        /*this.dSend(this.channel, [
          '< Popo\'s cute little bot has started listening to guild chat >',
          {
            code: 'md'
          }
        ]);*/
        ...
        /*this.dSend(this.channel, [
          '< Popo\'s cute little bot is going to rest for now >',
          {
            code: 'md'
          }
        ]);*/
          ...
          this.dSend(this.channel, [
            {
              embed: {
                ...
                image: {
                  url: 'https://cdn.discordapp.com/icons/489826398459002881/bc6d1cfaca06bf0e238e54ea1172bc1d.jpg'
                }
              }
            }
          ]);

...
class DiscordHandler {
  ...
  discordIPCToDiscordMsg() {
    ...
    this.ipc_discord.on('quest', (quests) => {
    ...
    this.discordBot.on('message', (message) => {
      ...
      switch (message) {
        case '.gc topic':
          ...
          break;
        case '.gc online':
          this.channel.send(this.guild.members);
          break;
        case '.gc motd':
          this.channel.send(this.guild.motd);
          break;
        case '.gc quests':
          this.channel.send(this.guild.quests);
          break;
        default:
          break;
      }
  ...
  updateTopic() {
    ...
    if (this.guild.quests.length > 0) {
      const quests = this.guild.quests.map((quest) => {
        let progress;

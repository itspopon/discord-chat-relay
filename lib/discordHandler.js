...
class DiscordHandler {
  ...
  discordMsgToTeraIPC() {
    this.discordBot.on('message', message => {
      ...
      switch (message.content) {
          ...
          this.channel.send({
            embed: {
              ...
              description: `\`\`\`md\n< MOTD >\n\n# ${
                ...
                ''
              )}\n< GQUESTS >\n\n${this.guild.quests.reduce(
                (allQuests, quest) =>
                  allQuests + `[${quest.name}](${quest.progress})\n`,
                ''
              )}\`\`\``,
        ...
        default:
          console.log('[discordHandler.js] Unknown case: ', message.content);
          break;
      }
  ...
  updateTopic() {
    ...
    parts.push(`< Last updated: ${new Date(Date.now()).toISOString()} >`);

...
class DiscordHandler {
  constructor(dispatch, config) {
    ...
    this.discordBot.on('ready', () => {
      ...
      this.gquestChannel = U.getTextChannel(this.server, this.config.channels.gquests);
      this.guildAnnouncementsChannel = U.getTextChannel(this.server, this.config.channels.guildannouncements);
      this.guildFirewallChannel = U.getTextChannel(this.server, this.config.channels.guildfirewall);
  ...
  initGchatModule() {
    ...
    if (!this.guildFirewallChannel) {
      console.warn('* guild firewall channel disabled');
    }
  ...
  discordIPCToDiscordMsg() {
    ...
    this.ipc_discord.on('sysmsg', (message, options = { codeStyle: undefined, logChan: undefined }) => {
      ...
      if (logChan) {
        if (logChan === this.config.channels.guildfirewall) {
          this.guildFirewallChannel.send(message, {
            code: codeStyle,
            disable_everyone: true
          });
        }

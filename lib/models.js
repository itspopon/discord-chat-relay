...
try {
  if (config.db.local) {
    mongoose.connect(`mongodb://localhost:27017/tera`, {
...
if (config.devMode) {
  ...
  TeraUnsorted = mongoose.model(
    'TeraUnsorted',
    AnyObject.anyobjectSchema,
    '_Unsorted'
  );
  TeraLogs = mongoose.model(
    'TeraLogs',
    Logs.logSchema,
    `_Logs_${U.getYearAndMonth()}`
  );
} else {
